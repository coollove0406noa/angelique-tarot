...
export type Card = {
  id: number;
  name: string;
  image: string;                // 画像パス（/public/cards 配下）
  upright?: string;             // 正位置の短文
  reversed?: string;            // 逆位置の短文
  advice?: {                    // フォールバック用（OneOracle が使う）
    all?: string;
    love?: string;
    work?: string;
  };
...
// 大アルカナ 22枚をまとめる
export const FULL_DECK: Card[] = Array.from({ length: 22 }, (_, i) => ({
  id: i,
  name: NAMES[i],
  image: `/cards/${String(i).padStart(2, "0")}_${SLUGS[i]}.svg`,
  upright: UPRIGHT[i],
  reversed: REVERSED[i],
  advice: { all: UPRIGHT[i] }, // フォールバック用
}));

// 互換用（過去コードで deck を参照していた場合）
